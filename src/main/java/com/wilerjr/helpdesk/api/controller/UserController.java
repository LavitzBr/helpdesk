package com.wilerjr.helpdesk.api.controller;
...
import com.wilerjr.helpdesk.api.response.Response;
...
public class UserController {
	...
	@PutMapping
	public ResponseEntity<Response<User>> update(HttpServletRequest request, @RequestBody User user, BindingResult result) { // @RequestBody User user // usuário que será modificado
		Response<User> response = new Response<User>();
		try {
	...
	private void validateUpdateUser(User user, BindingResult result) {
		if (user.getEmail() == null) {
			result.addError(new ObjectError("User", "No ID information"));
		}
		if (user.getEmail() == null) {
			result.addError(new ObjectError("User", "No email information"));
		}
	}
	...
	public ResponseEntity<Response<User>> findById(@PathVariable("id") String id) {
		...
		if (user == null) {
			response.getErrors().add("Register not found of id: " + id);
			return ResponseEntity.badRequest().body(response);
		}
		response.setDataOptional(user);
